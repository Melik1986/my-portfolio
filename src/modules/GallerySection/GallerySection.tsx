...
export function GallerySection(): ReactElement {
  const { displayedIndex, itemsToRender, nextSlide, prevSlide, carouselRef, listRef, progressRef } = useCarousel();
  ...
  return (
    <div className={styles.gallery} id="portfolio" ref={carouselRef}>
      ...
      <ul className={styles.gallery__list}>
        {itemsToRender.map(
          ({ className, title, name, description }: GalleryItem, index) => (
            <li
              key={`${className}-${index}`}
      ...
      <div className={styles.gallery__arrows}>
        <button className={styles.gallery__prev} onClick={prevSlide} title="Previous slide">
          <SpriteIcon id="arrow-left" name="icon-arrow-left" className={styles.gallery__arrowIcon} sprite="/images/icons/tech-icons.svg" />
        </button>
        <button className={styles.gallery__next} onClick={nextSlide} title="Next slide">
          <SpriteIcon id="arrow-right" name="icon-arrow-right" className={styles.gallery__arrowIcon} sprite="/images/icons/tech-icons.svg" />
        </button>
        <div className={styles.gallery__slideNumber}>{displayedIndex + 1}/{totalItems}</div>
      </div>
